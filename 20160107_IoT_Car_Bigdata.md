...
## なぜ、iOSデベロッパーの方が多いか？
* ハードウェアもソフトウェアも一括で作っているので
  -> アクサ損保と一緒にやるときに同じようにハードとソフトどちらもやることを

## プロトタイプとの違い
* 製造コストが大事
* 一つのミスで大きな損失  
  ハードの回収があると利益がなくなるので絶対になくす
...
* 対応車種の調査（セキュリティのため、外側からは物理的にアップデートされないようにしている）
...
* サーバ構成
  * 全てAWS
  * アプリケーションサーバ -> ローデータは全てDBに -> 定期的にWorkerが動く -> 集まっているIDのデータから計算を行う

  * APIサーバ：Rails
  * 解析処理：Node.js、Promise
  * DataStore：Amazon RDS（MySQL）

### データの特徴
* デバイスとスマホの2種類のデータ
  * 別々にアップされる
  * タイムスタンプを見て同じドライブのデータかどうか判定
* 向こうなデータのフィルタリング処理、有効値による補完処理
* 解析
  * 後からデータがアップされることを考慮
...
* 通信はどうしてスマホ経由とOBDから直接のにしたのか
  * BlueTooth+スマホだと費用が抑えれるため
  * OBDのデータ

* データの加工にあえて、RDBを使い、NoSQLを使わなかったのはなぜか
  * データの更新が多いため
...
* やばいと思ったこと
  * CANの知識を持っていたので簡単にできるんじゃないと思ったが、アクサ損保のキーパーソンの人の車種だけが不具合発生して焦った
